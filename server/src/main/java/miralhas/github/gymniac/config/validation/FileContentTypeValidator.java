package miralhas.github.gymniac.config.validation;
...
public class FileContentTypeValidator implements ConstraintValidator<FileContentType, List<MultipartFile>> {
	...
	public boolean isValid(List<MultipartFile> values, ConstraintValidatorContext context) {
		...
		var isValid = values.stream().anyMatch((file) -> acceptedMediaTypes.contains(file.getContentType()));
		if (!isValid) {
